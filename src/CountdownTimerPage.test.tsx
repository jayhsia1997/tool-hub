...
describe('Countdown Timer', () => {
  ...
  it('rejects a Target Time that has already passed', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(
      /target time must be later than now/i,
    )
    expect(screen.queryByRole('timer')).not.toBeInTheDocument()
  })
  ...
  it('hides a blank title and shows a provided title on the display', () => {
    ...
    applySettings()
    expect(
      within(screen.getByLabelText(/countdown display/i)).queryByRole(
        'heading',
      ),
    ...
    expect(
      within(screen.getByLabelText(/countdown display/i)).getByRole(
        'heading',
        { name: 'Break ends' },
      ),
    ).toBeInTheDocument()
  ...
  it('shows Completion Message at zero, or keeps 00:00:00 when blank', () => {
    ...
    expect(screen.queryByRole('timer')).not.toBeInTheDocument()
    expect(
      within(screen.getByLabelText(/countdown display/i)).getByText(
        'Time is up',
      ),
    ).toBeInTheDocument()
  })
